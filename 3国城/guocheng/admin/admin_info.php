...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000">
  ...
  <?
    ...
    if(mysql_select_db("gcgj")){
      ...
      $first=10*($npage-1);
      $sql="select * from information where name='招贤纳才' order by id desc limit $first,10";
      $result=mysql_query($sql);
...
<form method=GET action="admin_info.php">
      ...
      <td height="30" align="center">
      	<?
      		if($npage==1){
      			...
      			echo "<a href=admin_info.php?npage=1>首页</a>&nbsp;";
      			echo "<a href=admin_info.php?npage=".($npage-1).">上一页</a>&nbsp;";
      		}
      		if($npage==$maxpage){
      			echo "下一页 尾页";
      		}else{
      			echo "<a href=admin_info.php?npage=".($npage+1).">下一页</a>&nbsp;";
      			echo "<a href=admin_info.php?npage=".$maxpage.">尾页</a>";
      		}
