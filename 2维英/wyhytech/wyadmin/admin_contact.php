...
* ============================================================================
*/
define('ADMIN_PATH_WEB', str_replace('admin_contact.php', '', str_replace('\\', '/', __FILE__)));
define('PATH_WEB', dirname(ADMIN_PATH_WEB).'/');
require_once(dirname(__FILE__).'/../configs/config.php');
require_once(dirname(__FILE__).'/include/admin_common.inc.php');
require_once(ADMIN_ROOT_PATH.'include/admin_contact_fun.php');
...
if($act == 'list')
{
	check_permissions($_SESSION['admin_purview'],"contact_show");
	...
	$oederbysql=" order BY a.contact_order DESC,a.id DESC";
	if ($key && $key_type>0)
	{
	...
	$joinsql=" LEFT JOIN ".table('contact_category')." AS c ON a.type_id=c.id  ";
	$total_sql="SELECT COUNT(*) AS num FROM ".table('contact')." AS a ".$joinsql.$wheresql;
	$page = new page(array('total'=>$db->get_total($total_sql), 'perpage'=>$perpage));
	$currenpage=$page->nowindex;
	$offset=($currenpage-1)*$perpage;
	$contact = get_contact($offset, $perpage,$joinsql.$wheresql.$oederbysql);
	$smarty->assign('contact',$contact);
	$smarty->assign('page',$page->show(3));
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact.htm');
}
elseif($act =='migrate_contact')
{
	$id=$_REQUEST['id'];
	if (empty($id)) adminmsg("请选择项目！",1);
	check_token();
	check_permissions($_SESSION['admin_purview'],"contact_del");
	if (del_contact($id))
	{
		adminmsg("删除成功！",2);
	}
}
elseif($act == 'contact_add')
{
	check_permissions($_SESSION['admin_purview'],"contact_add");
	$smarty->assign('contact_category',get_contact_category());
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact_add.htm');
}
elseif($act == 'addsave')
{
	check_permissions($_SESSION['admin_purview'],"contact_add");
	...
	$setsqlarr['contact_order']=intval($_POST['contact_order']);
	form_imglist($setsqlarr['imgurl'],273,172,2,$thumb_dir.date('Ymd/'));
	$setsqlarr['addtime']=$timestamp;
	$setsqlarr['parentid']=get_contact_parentid($setsqlarr['type_id']);
	$link[0]['text'] = "继续添加";
	$link[0]['href'] = '?act=contact_add&type_id_cn='.trim($_POST['type_id_cn'])."&type_id=".$_POST['type_id'];
	$link[1]['text'] = "返回列表";
	$link[1]['href'] = '?act=list';
	!inserttable(table('contact'),$setsqlarr)?adminmsg("添加失败！",0):adminmsg("添加成功！",2,$link);
}
elseif($act == 'contact_edit')
{
	check_permissions($_SESSION['admin_purview'],"contact_edit");
	$id=intval($_GET['id']);
	$sql = "select * from ".table('contact')." where id=".intval($id)." LIMIT 1";
	$edit_contact=$db->getone($sql);

	$smarty->assign('edit_contact',$edit_contact);
	...
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact_edit.htm');
}
elseif($act == 'editsave')
{
	check_permissions($_SESSION['admin_purview'],"contact_edit");
	...
	$link[1]['text'] = "查看已修改";
	$link[1]['href'] = "?act=contact_edit&id=".$id;
	!updatetable(table('contact'),$setsqlarr," id=".$id."")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}

elseif($act == 'property'){
	check_permissions($_SESSION['admin_purview'],"contact_property");
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact_property.htm');
}

elseif($act == 'category')
{
	check_permissions($_SESSION['admin_purview'],"contact_category");
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact_category.htm');
}
elseif($act == 'category_add')
{
	...
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact_category_add.htm');
}
	...
	if (is_array($_POST['categoryname']) && count($_POST['categoryname'])>0)
	{
		for ($i =0; $i <count($_POST['categoryname']);$i++){
			if (!empty($_POST['categoryname'][$i]))
			{
				...
				!inserttable(table('contact_category'),$setsqlarr)?adminmsg("添加失败！",0):"";
...
elseif($act == 'edit_category')
{
	check_permissions($_SESSION['admin_purview'],"contact_category");
	$id=intval($_GET['id']);
	$smarty->assign('category',get_contact_category_one($id));
	$smarty->assign('pageheader',"联系我们");
	get_token();
	$smarty->display('contact/admin_contact_category_edit.htm');
}
elseif($act == 'edit_category_save')
{
	...
	!updatetable(table('contact_category'),$setsqlarr," id='".$id."'")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
?>