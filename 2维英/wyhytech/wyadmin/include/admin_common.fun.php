...
{
	global $db;
	$cache_file_path =ROOT_PATH. "configs/cache_page.php";
...
{
	global $db;
	$cache_file_path =ROOT_PATH. "configs/cache_points_rule.php";
...
{
	global $db;
	$cache_file_path =ROOT_PATH. "configs/cache_category.php";
...
{
	global $db;
	$cache_file_path =ROOT_PATH. "configs/cache_nav.php";
...
{
	global $db;
	$cache_file_path =ROOT_PATH. "configs/cache_subsite.php";
	$sql = "SELECT * FROM ".table('subsite')." WHERE s_effective=1";
	$arr = $db->getall($sql);
	foreach($arr as $key=> $val)
	{
		$config_arr[$val['s_domain']] =array("id"=>$val['s_id'],"sitename"=>$val['s_sitename'],"district"=>$val['s_district'],"districtname"=>$val['s_districtname'],"tpl"=>$val['s_tpl'],"logo"=>$val['s_logo'],"filter_notice"=>$val['s_filter_notice'],"filter_jobs"=>$val['s_filter_jobs'],"filter_resume"=>$val['s_filter_resume'],"filter_ad"=>$val['s_filter_ad'],"filter_links"=>$val['s_filter_links'],"filter_news"=>$val['s_filter_news'],"filter_explain"=>$val['s_filter_explain']);
	}
	write_static_cache($cache_file_path,$config_arr);
}
...
function makejs_classify()
{
	...
	$fp = @fopen(ROOT_PATH . 'configs/cache_classify.js', 'wb+');
	if (!$fp){
		exit('生成JS文件失败');
	}
	if (strcasecmp(DBCHARSET,"utf8")!=0)
	{
		$content=iconv(DBCHARSET,"utf-8//IGNORE",$content);
	}
	if (!@fwrite($fp, trim($content))){
		exit('写入JS文件失败');
	}
...
function form_imglist($imgurl,$width='',$height='',$kind='',$savepath){
	require_once('thumb.class.php');
	$thumb=new thumb();
	$thumb->list_simage($width,$height,$kind,$savepath);
	$imglist = explode("|",$imgurl);
	$i=0;
	$url='';
	foreach($imglist as $val){
		$i++;
		$smallimg= $thumb->createthumb(PATH_WEB.'upload/'.$val);
		$lt=$smallimg['path'];
		$url.=count($imglist)==$i?$lt:$lt.'|';
