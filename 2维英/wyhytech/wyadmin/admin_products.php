...
* ============================================================================
*/
define('ADMIN_PATH_WEB', str_replace('admin_products.php', '', str_replace('\\', '/', __FILE__)));
define('PATH_WEB', dirname(ADMIN_PATH_WEB).'/');
require_once(dirname(__FILE__).'/../configs/config.php');
require_once(dirname(__FILE__).'/include/admin_common.inc.php');
require_once(ADMIN_ROOT_PATH.'include/admin_products_fun.php');
...
if($act == 'list')
{
	check_permissions($_SESSION['admin_purview'],"products_show");
	require_once(ROOT_PATH.'include/page.class.php');
	$key=isset($_GET['key'])?trim($_GET['key']):"";
	$key_type=isset($_GET['key_type'])?intval($_GET['key_type']):"";
	$oederbysql=" order BY a.products_order DESC,a.id DESC";
	if ($key && $key_type>0)
	{
	...
	$joinsql=" LEFT JOIN ".table('products_category')." AS c ON a.type_id=c.id  ";
	$total_sql="SELECT COUNT(*) AS num FROM ".table('products')." AS a ".$joinsql.$wheresql;
	$page = new page(array('total'=>$db->get_total($total_sql), 'perpage'=>$perpage));
	$currenpage=$page->nowindex;
	$offset=($currenpage-1)*$perpage;
	$products = get_products($offset, $perpage,$joinsql.$wheresql.$oederbysql);
	$smarty->assign('products',$products);
	$smarty->assign('page',$page->show(3));
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products.htm');
}
elseif($act =='migrate_products')
{
	$id=$_REQUEST['id'];
	if (empty($id)) adminmsg("请选择项目！",1);
	check_token();
	check_permissions($_SESSION['admin_purview'],"products_del");
	if (del_products($id))
	{
		adminmsg("删除成功！",2);
	}
}
elseif($act == 'products_add')
{
	check_permissions($_SESSION['admin_purview'],"products_add");
	$smarty->assign('products_category',get_products_category());
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products_add.htm');
}
elseif($act == 'addsave')
{
	check_permissions($_SESSION['admin_purview'],"products_add");
	...
	$setsqlarr['products_order']=intval($_POST['products_order']);
	form_imglist($setsqlarr['imgurl'],224,386,2,$thumb_dir.date('Ymd/'));
	$setsqlarr['addtime']=$timestamp;
	$setsqlarr['parentid']=get_products_parentid($setsqlarr['type_id']);
	$link[0]['text'] = "继续添加产品";
	$link[0]['href'] = '?act=products_add&type_id_cn='.trim($_POST['type_id_cn'])."&type_id=".$_POST['type_id'];
	$link[1]['text'] = "返回产品列表";
	$link[1]['href'] = '?act=list';
	!inserttable(table('products'),$setsqlarr)?adminmsg("添加失败！",0):adminmsg("添加成功！",2,$link);
}
elseif($act == 'products_edit')
{
	check_permissions($_SESSION['admin_purview'],"products_edit");
	$id=intval($_GET['id']);
	$sql = "select * from ".table('products')." where id=".intval($id)." LIMIT 1";
	$edit_products=$db->getone($sql);

	$smarty->assign('edit_products',$edit_products);
	...
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products_edit.htm');
}
elseif($act == 'editsave')
{
	check_permissions($_SESSION['admin_purview'],"products_edit");
	...
	$link[1]['text'] = "查看已修改产品";
	$link[1]['href'] = "?act=products_edit&id=".$id;
	!updatetable(table('products'),$setsqlarr," id=".$id."")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
elseif($act == 'del_img')
{
	check_token();
	$id=intval($_GET['id']);
	$img=$_GET['img'];
	$sql="update ".table('products')." set Small_img='' where id=".$id." LIMIT 1";
	$db->query($sql);
...
elseif($act == 'property'){
	check_permissions($_SESSION['admin_purview'],"products_property");
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products_property.htm');
}

elseif($act == 'category')
{
	check_permissions($_SESSION['admin_purview'],"products_category");
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products_category.htm');
}
elseif($act == 'category_add')
{
	...
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products_category_add.htm');
}
	...
	if (is_array($_POST['categoryname']) && count($_POST['categoryname'])>0)
	{
		for ($i =0; $i <count($_POST['categoryname']);$i++){
			if (!empty($_POST['categoryname'][$i]))
			{
				...
				!inserttable(table('products_category'),$setsqlarr)?adminmsg("添加失败！",0):"";
...
elseif($act == 'edit_category')
{
	check_permissions($_SESSION['admin_purview'],"products_category");
	$id=intval($_GET['id']);
	$smarty->assign('category',get_products_category_one($id));
	$smarty->assign('pageheader',"产品展示");
	get_token();
	$smarty->display('products/admin_products_category_edit.htm');
}
elseif($act == 'edit_category_save')
{
	...
	!updatetable(table('products_category'),$setsqlarr," id='".$id."'")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
?>