...
{
	check_permissions($_SESSION['admin_purview'],"database");
	if (!file_exists("../configs/".$backup_dir."/"))adminmsg("备份文件存放目录configs/".$backup_dir."不存在！",0);
	if (!is_writable("../configs/".$backup_dir."/"))adminmsg("备份文件存放目录configs/".$backup_dir."不可写！",0);
	...
	if (!empty($_POST['tables']))
	{
		...
		@file_put_contents("../configs/{$backup_dir}/temp.txt", serialize($_POST['tables']));
	}
	elseif ($_GET['table_id'])
	{
		$content = file_get_contents("../configs/{$backup_dir}/temp.txt");
	...
	for($i = $table_id; $i < $count; $i++)
	{
		...
		if ($pos == 0)
		{
			...
			{
				$s = "ENGINE=MyISAM DEFAULT CHARSET=".SYS_CHARSET.";\r\n";
				$table_sql = $table_sql1 . $s;
			}
			else
			{
		...
		while ($row = $db->fetch_array($result, MYSQL_NUM))
		{
			...
			{
				if (!write_file("../configs/{$backup_dir}/{$file}_{$num}.sql", $sql))
				{
		...
		elseif ($i == $count-1)
		{
			...
			@unlink("../configs/{$backup_dir}/temp.txt");
			$link[0]['text'] = "查看备份文件";
...
elseif($act =='restore')
{
	get_token();
	check_permissions($_SESSION['admin_purview'],"database");
	if (!file_exists("../configs/{$backup_dir}/"))adminmsg("备份文件存放目录configs/backup不存在！",0);
	$data_backup_list = $file_info = array();
	$dir = opendir('../configs/'.$backup_dir);
	...
	{
		if (file_exists("../configs/".$backup_dir."/".$file))
		{
			$sqlfile_info_arr = get_sqlfile_info("../configs/".$backup_dir."/".$file);
			$file_info[$key]['file_size'] = round(filesize("../configs/".$backup_dir."/".$file)/1024/1024,2);
		}
		else
		{
			$sqlfile_info_arr = get_sqlfile_info("../configs/".$backup_dir."/".preg_replace('#\.sql#i','',$file).'_1.sql');
			$i = 1;
			while (file_exists("../configs/".$backup_dir."/".preg_replace('#\.sql#i','',$file).'_'.$i.'.sql'))
			{
				$file_info[$key]['file_size'] += round(filesize("../configs/{$backup_dir}/".preg_replace('#\.sql#i','',$file)."_{$i}.sql")/1024/1024,2);
				$i++;
			}
		}
...
elseif($act =='del')
{
	...
	foreach ($file_name as $fname)
	{
		...
		if ($handle = opendir("../configs/".$backup_dir))
		{
...
elseif($act =='import')
{
	...
	closedir("../configs/".$backup_dir);
	$file ="../configs/{$backup_dir}/{$backup_file[0]}";
	...
	{
		$file = "../configs/{$backup_dir}/{$backup_file}";
		...
		for($i = 0; $i < $arr_count; $i++)
		{
			...
			if (!empty($arr[$i]))
			{
				if ((strpos($arr[$i], 'CREATE TABLE') !== false) && (strpos($arr[$i], 'DEFAULT CHARSET='.str_replace('-', '', SYS_CHARSET) )!== false))
				{
					$arr[$i] = str_replace('DEFAULT CHARSET='. str_replace('-', '', SYS_CHARSET), '', $arr[$i]);
				}
