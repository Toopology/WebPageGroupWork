<?php
function tpl_function_train_list($params, &$smarty)
{
	global $db,$_CFG;
	...
	if ($aset['displayorder'])
	{
		if (strpos($aset['displayorder'],'>'))
		{
			...
			$arr[0]=preg_match('/train_order|click|id/',$arr[0])?$arr[0]:"";
			$arr[1]=preg_match('/asc|desc/',$arr[1])?$arr[1]:"";
			if ($arr[0] && $arr[1])
			{
				$orderbysql=" ORDER BY ".$arr[0]." ".$arr[1];
			}
			if ($arr[0]=="train_order")
			{
				$orderbysql.=" ,id DESC ";
			}
		}
	...
	if (isset($aset['paged']))
	{
		...
		$total_sql="SELECT COUNT(*) AS num FROM ".table('train').$wheresql;
	...
	$result = $db->query("SELECT * FROM ".table('train')." ".$wheresql.$orderbysql.$limit);
	$list= array();
	$i=0;
