<?php
function tpl_function_products_list($params, &$smarty)
{
	...
	$aset['showname']=isset($aset['showname'])?$aset['showname']:'productsshow';
	if ($aset['displayorder'])
	{
		if (strpos($aset['displayorder'],'>'))
		{
			$arr=explode('>',$aset['displayorder']);
			$arr[0]=preg_match('/products_order|click|id/',$arr[0])?$arr[0]:"";
			$arr[1]=preg_match('/asc|desc/',$arr[1])?$arr[1]:"";
			if ($arr[0] && $arr[1])
			{
				$orderbysql=" ORDER BY ".$arr[0]." ".$arr[1];
			}
			if ($arr[0]=="products_order")
			{
				$orderbysql.=" ,id DESC ";
			}
		}
	...
	if (isset($aset['paged']))
	{
		...
		$total_sql="SELECT COUNT(*) AS num FROM ".table('products').$wheresql;
	...
	$result = $db->query("SELECT * FROM ".table('products')." ".$wheresql.$orderbysql.$limit);
	$list= array();
	$i=0;
