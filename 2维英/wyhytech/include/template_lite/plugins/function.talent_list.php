<?php
function tpl_function_talent_list($params, &$smarty)
{
	global $db,$_CFG;
	...
	$aset['showname']=isset($aset['showname'])?$aset['showname']:'newsshow';
	if ($aset['displayorder'])
	{
		if (strpos($aset['displayorder'],'>'))
		{
			$arr=explode('>',$aset['displayorder']);
			$arr[0]=preg_match('/about_order|click|id/',$arr[0])?$arr[0]:"";
			$arr[1]=preg_match('/asc|desc/',$arr[1])?$arr[1]:"";
			if ($arr[0] && $arr[1])
			{
				$orderbysql=" ORDER BY ".$arr[0]." ".$arr[1];
			}
			if ($arr[0]=="about_order")
			{
				$orderbysql.=" ,id DESC ";
			}
		}
	...
	if (isset($aset['paged']))
	{
		...
		$total_sql="SELECT COUNT(*) AS num FROM ".table('about').$wheresql;
		$total_count=$db->get_total($total_sql);
		$pagelist = new page(array('total'=>$total_count, 'perpage'=>$aset['row'],'alias'=>'talent','id0'=>$aset['type_id']));
	...
	$result = $db->query("SELECT * FROM ".table('about')." ".$wheresql.$orderbysql.$limit);
	$list= array();
	$i=0;
	while($row = $db->fetch_array($result))
	{
		...
		$row['bimg']=$_CFG['upfiles_dir'].$row['small_imgurl'];
		$row['num']=$i;
		$list[] = $row;
	}
